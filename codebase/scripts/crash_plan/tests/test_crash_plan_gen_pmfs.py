...
def main(argc, argv):
    ...
    pmfs_cheat = CheatSheetPmfs()
    undo_jnl_cheat = MechUndoJnlCheat(trace_reader, stinfo_index, pmfs_cheat)
    lsw_cheat = MechLSWCheat(trace_reader, stinfo_index, pmfs_cheat)
    ...
    for split_entry in split_op_mgr.op_entry_list:
        ...
        # generate in-flight cluster map again with file data write
        cache = WitcherCache(EmptyBinaryFile())
        split_entry.convert_atomic_ops(include_val_bytes=False,
                                       ignore_user_data_write=False,
                                       ignore_copies=False,
                                       ignore_memset=False,
                                       sampling_non_atomic_write=False,
                                       force=True)
        split_entry.analysis_in_cache_run(cache)
        data = split_entry.get_cache_analysis_result()
        print(data)
        # fname = "%s/%s-%d-perf.ana.user.copies.memset.result" % (dir_name, split_entry.op_name, count)
        ...
        lsw_reason = MechLSWReason(split_entry, stinfo_index, pmstore_reason,
                                   cstore_reason, undojnl_reason, link_reason, lsw_cheat)
        rep_reason = None
