...
std::string byte_to_hex_string(const byte* arr) {
  string str;
  str.reserve(16 << 1);
...
MD5::MD5(const byte* message, size_t length) {
  finished = false;
  /* Reset number of bits. */
  count[0] = count[1] = 0;
  /* Initialization constants. */
...
string MD5::toStr() {
  ...
  return byte_to_hex_string(digest_);
}


extern "C" {
  void *compute_md5(byte *content, size_t size) {
    MD5 md5(content, size);
    std::string digest = md5.toStr();

    char *ret = (char*)malloc(33);
    if (!ret) {
      return NULL;
    }
    memcpy(ret, digest.c_str(), digest.size());
    ret[32] = '\0';
    fprintf(stderr, "addr: 0x%p\n", ret);
    return (void*)ret;
  }

  void free_md5_string(void *s) {
