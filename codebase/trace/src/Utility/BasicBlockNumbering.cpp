...
MDNode* BasicBlockNumberPass::assignIDToBlock (BasicBlock *BB, unsigned id) {
  ...
  */
  Metadata *meta[2];
  meta[0] = ValueAsMetadata::get(&BB->front());
...
bool QueryBasicBlockNumbers::runOnModule(Module &M) {
  ...
  for (unsigned index = 0; index < MD->getNumOperands(); ++index) {
    ...
    BasicBlock *BB = (dyn_cast<Instruction>((dyn_cast<ValueAsMetadata>(Node->getOperand(0)))->getValue()))->getParent();
    ...
    bool inserted = IDMap.insert({BB, id}).second;
    assert(inserted && "Repeated BB!\n");

    inserted = BBMap.insert({id, BB}).second;
    if (inserted == false) {
      outs() << id << "\n";
      assert(inserted == true && "Repeated identifier!");
    }
  }
