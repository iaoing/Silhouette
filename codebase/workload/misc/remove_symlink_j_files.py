'''
Remove j-lang files that contain the symlink operation.
...
def removeSymlinkJFile(thd_id, j_file_dir, j_lang_file_list, lo, hi):
    for idx in tqdm(range(lo, hi), desc=f"thd: {thd_id}"):
        ...
        if flag:
            # print(f"remove {jfile}", file=sys.stderr)
            os.remove(jfile)
...
def main(args):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=cpus) as executor:
        ...
        future_to_task = {executor.submit(removeSymlinkJFile, i, j_file_dir, j_lang_file_list,
