...
def build_parser():
    ...
    parser.add_argument('--gencpp', '-c', default='False', help='Generate CPP source files?')
    parser.add_argument('--log', '-g', default='False', help='Dump log to disk?')
...
def print_setup(parsed_args):
    ...
    print('{0:20}  {1}'.format('GenCPP', parsed_args.gencpp))
    print('{0:20}  {1}'.format('Log', parsed_args.log))
...
def buildTuple(command, expand_combinations=True):
    ...
    elif command == 'fdatasync' or command == 'fsetxattr' or command == 'removexattr' or command == 'chmod':
        d = tuple(FileOptions)
    elif command == 'fsync':
...
def insertChmod(file_name, open_dir_map, open_file_map, file_length_map, modified_pos):
    return ('chmod', file_name)
...
#       elif option == 'overlap_unaligned_start' or 'overlap_unaligned_end' or 'overlap_start' or 'overlap_end' or 'overlap_extend':
    ...
    elif command == 'fsync' or command == 'fdatasync' or command == 'fsetxattr' or command == 'chmod':
        modified_pos = checkParentExistsDep(current_sequence, pos, modified_sequence, modified_pos, open_dir_map, open_file_map, file_length_map)
...
def buildJ2lang(sequence):
    ...
    for op, parameters in sequence:
        ...
        elif (op == "unlink" or op == "remove" or op == "fsetxattr" or op == "removexattr" or op == "fdatasync" or op == "chmod"):
            f1 = new_file(parameters[0])
...
def buildJlang(op_list, length_map, test_type):
    ...
    if command == 'unlink'or command == 'remove' or command == 'rmdir' or command == 'close' or command == 'fsetxattr' or command == 'removexattr' or command == 'chmod':
        file = flat_list[1]
...
def doPermutation(perm, test_type):
    ...
    assert test_type == "pm", "does not support other type"
    ...
    # copy base file to dest dir
    if test_type == "pm":
        dest_j_fname = "../tests/" + dest_dir +  "/base-j-lang"
        base_j_fname = "../tests/ace-base/base-j-lang"
        copyfile(base_j_fname, dest_j_fname)

        dest_cpp_fname = "../tests/" + dest_dir +  "/base.cpp"
        base_cpp_fname = "../tests/ace-base/base.cpp"
        copyfile(base_cpp_fname, dest_cpp_fname)

        dest_makefile = "../tests/" + dest_dir + "/Makefile"
        base_makefile = "../tests/ace-base/Makefile"
        copyfile(base_makefile, dest_makefile)
...
def main(parsed_args):
    ...
    if parsed_args.gencpp == ('True' or 'true'):
        gencpp = True
    else:
        gencpp = False

    if parsed_args.log == ('True' or 'true'):
        g_log = True
    else:
        g_log = False
