...
def insertCreateSnapshot(contents, line, index_map, method):
    name = 'fd_procfs'
    decl = ' '
...
def insertDeleteSnapshot(contents, line, index_map, method):
    name = 'fd_procfs'
    decl = ' '
    if name not in redeclare_map:
        decl = 'int '
    ...
    with open(file, 'r+') as insert:
        ...
        #     insertLink(contents, option, line, index_map, method)
            ...
            if method == 'setup':
                ...
                updateRunMap(index_map, 1)
            insertCreateSnapshot(contents, line, index_map, method)
            ...
            if method == 'setup':
                ...
                updateRunMap(index_map, 1)
            insertDeleteSnapshot(contents, line, index_map, method)
...
def main():
    ...
    val = 0
    new_file = os.path.basename(test_file)  + ".cpp"
