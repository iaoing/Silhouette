	...
	struct snapshot_list *list)
{
	...
	while (curr_block) {
		...
		// kfree(curr_page);
		free_page((unsigned long)curr_page);
		freed++;
	}
	...
	struct snapshot_info *info, u64 epoch_id)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		if (!new_page || ENTRY_LOC(new_page)) {
			nova_dbg("allocate a new DRAM page failed: %llu, 0x%x", PAGE_SIZE, new_page);
			nova_dbg("%s: failed\n", __func__);
			// kfree((void *)new_page);
	...
	struct snapshot_info **ret_info, int init_pages, u64 epoch_id)
{
	...
	if (!info->lists) {
		nova_dbg("allocate info list failed.");
...
fail:
	for (i = 0; i < sbi->cpus; i++) {
		...
		if (list->head)
			...
			free_page((unsigned long)list->head);
	}
