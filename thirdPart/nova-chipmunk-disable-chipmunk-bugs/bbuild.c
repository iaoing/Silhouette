...
static int nova_init_inode_list_from_inode(struct super_block *sb)
{
	...
	curr_p = sih.log_head;
	nova_info("%s: curr_p: %llu\n", __func__, curr_p);
	if (curr_p == 0) {
		nova_dbg("%s: pi head is 0!\n", __func__);
		return -EINVAL;
	}

	nova_info("%s: curr_p: %llu, log_tail: %llu\n", __func__, curr_p, sih.log_tail);
	while (curr_p != sih.log_tail) {
		if (is_last_entry(curr_p, size))
			curr_p = next_log_page(sb, curr_p);
...
out:
	nova_info("%s: before free inode log\n", __func__);
	nova_free_inode_log(sb, pi, &sih);
	nova_info("%s: after free inode log\n", __func__);
	return ret;
}
...
again:
	...
	nova_dbg_verbose("Log head 0x%llx, tail 0x%llx\n",
				curr_p, pi->log_tail);
// #define CONFIG_NOVA_BUG3
#ifdef CONFIG_NOVA_BUG3 // do not enable this bug, since it blocks other tests
...
// #define CONFIG_NOVA_BUG1
#ifdef CONFIG_NOVA_BUG1  // do not enable this bug, since it blocks other tests
					&fake_pi, global_bm[1]);
#else
					&fake_pi, global_bm[0]);
#endif
...
static bool nova_try_normal_recovery(struct super_block *sb)
{
	...
	nova_info("end of init inode list from inode\n");
