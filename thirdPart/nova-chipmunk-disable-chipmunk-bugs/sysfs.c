	...
	size_t len, loff_t *ppos)
{
	...
	_buf[len] = 0;
	if (sscanf(_buf, "%llu", &epoch_id) == 1)
		nova_delete_snapshot(sb, epoch_id);
	else
		nova_warn("Couldn't parse delete_snapshot request: %s", _buf);
	...
	size_t len, loff_t *ppos)
{
	...
	if (sscanf(_buf, "%u:%u:%zu:%u", &func_id, &poolmb, &size, &disks) == 4)
		nova_test_perf(sb, func_id, poolmb, size, disks);
	else
		nova_warn("Couldn't parse test_perf request: %s", _buf);
