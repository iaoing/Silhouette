...
// #define CONFIG_NOVA_FORTIS_BUG2
	...
	struct nova_inode_info_header *sih, u64 head)
{
	...
	nova_info("nova_free_contiguous_log_blocks: curr_block: %llu\n", curr_block);
	while (curr_block > 0) {
	...
	nova_info("nova_free_contiguous_log_blocks: start_blocknr: %llu\n", start_blocknr);
	if (start_blocknr) {
	...
	struct nova_inode_info_header *sih)
{
	...
	nova_info("log head or log tail is 0: %llu, %llu\n", sih->log_head, sih->log_tail);
	...
	if (pi) {
		...
		nova_update_inode_checksum(pi);
		if (metadata_csum) {
			nova_info("before get alter_pi_addr: %llu, 0x%x\n", sih->alter_pi_addr, sih->alter_pi_addr);
			alter_pi = (struct nova_inode *)nova_get_block(sb,
						sih->alter_pi_addr);
			nova_info("after get alter_pi_addr: %llu, 0x%x\n", sih->alter_pi_addr, sih->alter_pi_addr);
			if (alter_pi) {
				nova_info("before memcpy alter_pi: 0x%x, pi: 0x%x\n", alter_pi, pi);
				memcpy_to_pmem_nocache(alter_pi, pi,
						sizeof(struct nova_inode));
				nova_info("after memcpy alter_pi: 0x%x, pi: 0x%x\n", alter_pi, pi);
			}
		}
		nova_memlock_inode(sb, pi, &irq_flags);
	}

	nova_info("start of nova_free_contiguous_log_blocks");
	freed = nova_free_contiguous_log_blocks(sb, sih, sih->log_head);
	if (metadata_csum)
		nova_info("start of checksum nova_free_contiguous_log_blocks");
		freed += nova_free_contiguous_log_blocks(sb, sih,
					sih->alter_log_head);
		nova_info("end of checksum nova_free_contiguous_log_blocks");

	nova_info("end of nova_free_contiguous_log_blocks");
